...
def exchange2(edgeList,dest,src):
	for e  in edgeList:
		if e[0]==dest:
			e[0]=src
...
def merge():
	...
	for i in range (1,p):
		name=a+str(i)
		B.add_node(name,cpu=1,typ='app',io=0,valid=1,app=[i],data=[],exio=0,ssd=0)		# we have to fill in the CPU, we have assigned all 1
	...
	#B.add_node('a4',cpu=1,typ='app',io=0,valid=1,app=[4],data=[],exio=0)
        ...
        for i in range (1,q):
                name=c+str(i)
		#print(name)
                B.add_node(name,cpu=0,typ='data',io=0,valid=1,app=[],data=[i],exio=0,ssd=0)         # io we will initialize 0 then update based on sum over edgelist
	#print(B.nodes.data())
	...
	seed2=5
        random.seed(seed2)
	total_storage_server_node_count=int(total_node_fat_tree*total_ssd_in_rack)
	Matrix1 = [ 0 for x in range(total_storage_server_node_count)]
	Matrix2 = [[] for x in range(total_storage_server_node_count)] 
	#print Matrix1
	for st in range(1,chunk_copy_number):
		for (v,wt) in B.nodes(data='io'):
			name='c'+str(st)
			if v==name :
				ssd=0
				while ssd % total_ssd_in_rack==0 or Matrix1[ssd]+wt>budget:
					ssd=random.randint(0,total_storage_server_node_count-1)
				if Matrix2[ssd]==[]:
					Matrix2[ssd].append(st)	
					Matrix1[ssd]=Matrix1[ssd]+wt
				else:
					u='c'+str(Matrix2[ssd][0])
					data_src=copy_content(B,u,v,'data')
					B.nodes[u]['data']=data_src
					B.nodes[v]['valid']=0
					B.nodes[u]['io']=B.nodes[u]['io']+B.nodes[v]['io']
					Matrix1[ssd]=Matrix1[ssd]+wt
				B.nodes[v]['ssd']=u	
				#print Matrix2
				#print "\n"
	...
	for (u,v,wt) in B.edges(data='weight'):			
		if  B.nodes[v]['valid']==0:
			B.remove_edge(u,v) 
			connected_node=B.nodes[v]['ssd']
			B.add_edge(u,connected_node, weight=wt , visited=0 )
	for (u,v,wt) in B.edges(data='weight'):
		sum_weight=find_parallel_edges(B,u,v)
		assert(sum_weight==wt)
	...
	while B.number_of_edges() !=0 :
		...
		dest_type= B.nodes[dest]['typ']
                #print(B.edges.data())
#------------------------------------------------treatment for data---------------------------------------
                if B.nodes[src]['valid']==1 and src[0]=='a' :
 		        	#print B.nodes[dest]['ssd']#=B.nodes[dest]['ssd']
				#if B.nodes[dest]['valid']==1:
				B.nodes[dest]['exio']=B.nodes[dest]['exio']+(B.nodes[src]['exio']-wt)
				B.nodes[src]['valid']=0
				app_src=copy_content(B,src,dest,'app')
				B.nodes[dest]['app']=app_src
				B.remove_edge(src,dest) 
                		for (u,v,wt) in B.edges(data='weight'):   # important
                        		if u==src:
						f=0
						for e in edgeList:
							if dest==e[0] and v==e[1]:
								e[2]=e[2]+wt
						...
						if f==0:
							B.remove_edge(u,v) 
                					B.add_edge(dest,v, weight=wt , visited=0 )
				#else:
					#node_copied_to=B.nodes[dest]['ssd']
					#B.nodes[node_copied_to]['exio']=B.nodes[node_copied_to]['exio']+(B.nodes[src]['exio']-wt)
					#B.nodes[src]['valid']=0
					#app_src=copy_content(B,src,node_copied_to,'app')
					#B.nodes[node_copied_to]['app']=app_src
					#B.remove_edge(src,dest) 
		else:  #the application has already been placed somewhere
				L=[]
                                L.append(src)
                                L.append(dest)
                                L.append(wt)
